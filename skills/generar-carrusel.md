...
## Input
`client_id`, brief, `idioma`, `N` (slides totales, típico 4), claim, bullets[], cta, marcas_externas[], persona (opcional).
...
5. **Slide 1 (cover):** ensamblar prompt con `prompts/template-prompt-completo.md`. `ESCENA` describe persona + escena (sin texto, sin logo). `COMPOSICIÓN` deja tercio para overlay. Generar AI. Descargar a `_render/cover-bg.png`. Componer `_render/slide1.html` (logo real + headline en tipografía de identity). Render con `tools/render-slide.ps1 -Width {W} -Height {H}`.
